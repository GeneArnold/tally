...
export default function DiaryDateNav({ date, prevDate, nextDate, isToday, displayLabel, displayDate }: Props) {
  ...
  return (
    <div className="flex items-center justify-between mb-4">
      <Link href={`/diary/${prevDate}`} className="text-blue-600 font-medium min-w-[44px] min-h-[44px] flex items-center">
        &larr; Prev
      </Link>
      <button
      ...
      >
        ...
        <span className="text-xs text-blue-600">{displayDate} &#x25BE;</span>
        <input
          ref={inputRef}
          type="date"
          value={date}
      ...
      <Link href={`/diary/${nextDate}`} className="text-blue-600 font-medium min-w-[44px] min-h-[44px] flex items-center">
        Next &rarr;
      </Link>
    </div>
  );
